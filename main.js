function Winwheel(options, drawWheel) {
  defaultOptions = {
    ...
    scaleFactor: 0.7, // Set by the responsive function. Used in many calculations to scale the wheel.
  };
  ...
  if (this.textMargin === null) {
    this.textMargin = this.textFontSize / 10;
  }
...
Winwheel.prototype.drawPins = function () {
  if (this.pins && this.pins.number) {
    ...
    let centerX = this.centerX * 1;
    let centerY = this.centerY * 1;
    let outerRadius = this.outerRadius * 1;
    ...
    if (this.pins.responsive) {
      pinOuterRadius = this.pins.outerRadius * 1;
      pinMargin = this.pins.margin * 1;
    }
...
Winwheel.prototype.drawWheelImage = function () {
  ...
  if (this.wheelImage != null) {
    ...
    let scaledWidth = this.wheelImage.width * 1;
    let scaledHeight = this.wheelImage.height * 1;
    ...
    if (this.segments) {
      ...
      for (let x = 1; x <= this.numSegments; x++) {
        ...
        if (seg.imgData.height) {
          ...
          let scaledWidth = seg.imgData.width * 1;
          let scaledHeight = seg.imgData.height * 1;
...
Winwheel.prototype.drawSegmentText = function () {
  ...
  if (this.ctx) {
    ...
    let centerY = this.centerY * 1;
    let outerRadius = this.outerRadius * 1;
    let innerRadius = this.innerRadius * 1;
...
const createLottie = () => {
  let openButton = document.getElementById("wheelButton");
  openButton.innerHTML = `<img id='wheelIcon' src='https://web-tools-sw.s3.eu-west-3.amazonaws.com/wheel/wheelIcon.gif' />`;
};

const createWheelButton = () => {
  const wheelButton = document.createElement("div");
  wheelButton.id = "wheelButton";
  document.body.appendChild(wheelButton);
};

let wheelData = null;
const fetchWheelData = async () => {
  const response = await fetch(
    "https://phoenix.herokuapp.com/wheel/63cba65e0fff48122356bd56",
    {
      method: "GET",
      headers: {
...
if (!localStorage.getItem("is_wheel")) {
  fetchWheelData();
}

const getModalContent = () => {
  let openModalBtn = document.getElementById("wheelButton");
  let closeModalBtn = document.getElementById("wheel_overlay");
  ...
  const handleSubmit = (e) => {
    ...
    if (emailInput.value && phoneInput.value) {
      if (!checkBox.checked) {
        return alert("Devam etmek için şartları kabul etmelisiniz.");
      }
      ...
      document.querySelector(".wheel-first-modal").style.display = "none";
      document.querySelector(".wheel-modal-main").style.display = "block";
    } else {
      return alert("Lütfen bilgilerinizi eksiksiz giriniz.");
    }
  };

  closeBtn.addEventListener("click", () => {
    document.getElementById("wheelModal").style.display = "none";
    document.getElementById("wheel_overlay").style.display = "none";
  });
  ...
  copyBtn.addEventListener("click", () => {
    if (resultText.innerText === "?????")
      return alert("Çarkı döndürmeden kopyalayamazsınız.");
  ...
  if (openModalBtn) {
    openModalBtn.addEventListener("click", () => {
      ...
      document.getElementById("wheel_overlay").style.display = "block";
    });
  }

  let theWheel = new Winwheel({
    numSegments: wheelData.length, // Specify number of segments.
  ...
  function sendCoupon(coupon) {
    const data = JSON.parse(localStorage.getItem("wheel"));
    ...
    const body = {
      ...
      coupon,
      analytic_type: "wheel",
      user_id: "63cba65e0fff48122356bd56",
    };
    fetch("https://phoenix.herokuapp.com/analytics", {
      method: "POST",
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem("is_wheel", true);
      }
    });
  }
  ...
  function alertPrize(indicatedSegment) {
    resultText.innerHTML = indicatedSegment.text;
    sendCoupon(indicatedSegment.text);
    openModalBtn.style.display = "none";
  }
};
window.onload = () => {
  let isWheel = localStorage.getItem("is_wheel");
  if (isWheel) {
    return;
  }
  ...
  loadStyle("https://cdn.jsdelivr.net/gh/serkancagman/wheel-testt/style.css");
  createWheelButton();
  createLottie();

  setTimeout(() => {
    const openModalBtn = document.getElementById("wheelButton");
    openModalBtn.addEventListener("click", () => {
      const modal = document.createElement("div");
      modal.id = "wheelModal";
      modal.innerHTML = ` <div id="close_modal" class="wheel-modal-close">
      <img
        src="https://cdn-icons-png.flaticon.com/64/1828/1828778.png"
        class="wheel-modal-close-img"
      />
    </div>
    <div class="wheel-modal-content">
      <div class="wheel-content-overlay"></div>
      <div class="wheel-modal-main">
        <div class="wheel-modal-main-inner">
          <canvas id="canvas" width="350" height="350"></canvas>
          <div class="wheel-result-area">
            <div class="wheel-result">
              <div class="wheel-result-inner">
                <h3 class="wheel-result-title">Kuponunuz</h3>
                <div class="wheel-result-text-area">
                  <span id="result_text" class="result-text">?????</span>
                  <button class="copy_btn" id="copy_button">Kopyala</button>
                </div>
              </div>
              <button id="spin_button" class="spin_btn">Çarkı Çevir!</button>
            </div>
          </div>
        </div>
      </div>
      <div class="wheel-first-modal">
        <div class="wheel-first-modal-content">
          <div class="wheel-first-modal-inner">
            <h3 class="wheel-first-modal-title">
              Çarkı kullanabilmek için mail adresinizi ve telefon numaranızı
              girmelisiniz.
            </h3>
            <form action="" class="wheel-first-modal-form" id="info-form">
              <div class="wheel-first-modal-form-group">
                <label for="email" class="wheel-first-modal-form-label"
                  >E-Posta</label
                >
                <input
                  type="email"
                  class="wheel-first-modal-form-input"
                  id="email"
                />
              </div>
              <div class="wheel-first-modal-form-group">
                <label for="phone" class="wheel-first-modal-form-label"
                  >Telefon</label
                >
                <input
                  type="text"
              ...
              <div id="termsCheck">
                ...
                <a href="#" id="wheelTermsLink" target="_blank" rel="noreferrer noopener">
              ...
              <div class="wheel-first-modal-form-group">
                <button
                  id="save-info-btn"
                  type="button"
                  class="wheel-first-modal-form-btn"
                >
                  Çarka Git
                </button>
              </div>
      ...
      if (!document.getElementById("wheel_overlay")) {
        const wheelOverlay = document.createElement("div");
        wheelOverlay.id = "wheel_overlay";
        wheelOverlay.classList.add("wheel-modal-overlay");
